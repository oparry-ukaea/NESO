...
void Outflow1DSystem::explicit_time_int(
  ...
  auto bcs = m_fields[0]->GetBndConditions();
  for (auto region_id = 0; region_id < bnd_exp_arr.size(); region_id++) {
    ...
    std::cout << "Region " << region_id << " has:" << std::endl
              ...
              << "  " << exp_list->GetTotPoints() << " trace points"
              << std::endl;

    if (bcs[region_id]->GetBoundaryConditionType() !=
      ...
      int region_last_idx_in_trace_arr =
          region_first_idx_in_trace_arr + exp_list->GetTotPoints();
      std::cout << "  and has indices " << region_first_idx_in_trace_arr
                << " to " << region_last_idx_in_trace_arr << " in the trace arr"
                << std::endl;
